...
CROSS_LDFLAGS = -L/usr/aarch64-linux-gnu/lib -lwiringPi -lcrypt -lpthread -lm -ldl -Wl,-rpath-link=/usr/aarch64-linux-gnu/lib -Wl,-rpath=.
...
# 소스 파일 정의
SRCS = main.c handle_lib.c thd_func.c ctrl_func.c
OBJS = $(SRCS:.c=.o)
...
librasp.so: librasp/librasp.c librasp/librasp.h
	$(CROSS_CC) $(CROSS_CFLAGS) -fPIC -shared -o $@ $< -lwiringPi

# 서버 빌드 
$(TARGET): $(OBJS) librasp.so
	$(CROSS_CC) $(CROSS_CFLAGS) -o $@ $(OBJS) $(CROSS_LDFLAGS)

%.o: %.c daemon_server.h handle_lib.h thd_func.h ctrl_func.h librasp/librasp.h
